...
def plot_embedding(data, path):
	...
	print(data.shape[0])
	random_arr = np.random.choice(data.shape[0],2000,replace=False)
...
def run(flag):
	...
	if not os.path.exists(name+'.npz'):
		...
		print("K=",cfg.K,"N=",cfg.N,"N+=",cfg.POS_SAMPLE_NUM,"embd_width=",cfg.EMBEDDING_WIDTH,"batch_size=",cfg.BATCH_SIZE,'margin=',cfg.MARGIN)
